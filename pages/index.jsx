...
const Home = ({ categories }) => {
					...
					<Link href={`/links/${slug}`}>
						<a className="flex items-center">
							<div className="w-16 h-16 mr-6 flex items-center justify-center">
								<img src={image && image.url} alt={name} className="w-full h-full"/>
							</div>
			...
			({
				...
				>
					<div>
						<div onClick={(e) => handleClick(_id)}>
							<a href={url} target="_blank">
								<h5 className="mb-2 font-semibold hover:text-red-400 text-lg">
									{title}
								</h5>
						...
						<div>
							...
							<span className="text-sm rounded py-1 px-2 text-red-400 bg-pink-100">
								{medium}
							</span>
					...
					<div className="flex flex-col justify-between mt-6 sm:mt-0">
						...
						<div className="flex justify-start sm:justify-end text-sm mt-4 sm:mt-0">
							{clicks} clicks
						</div>
					</div>
				</div>
	...
	return (
		<>
			<div className="bg-primary px-0 md:px-16 pt-24 pb-2">
				<div>
			...
			<div className="py-10 px-0 md:px-32 text-gray-600">
